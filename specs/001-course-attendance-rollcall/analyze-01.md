...
**Run date**: 2026-05-19
**Artifacts analyzed**: `spec.md`, `plan.md`, `tasks.md`, `data-model.md`, `research.md`, `contracts/`, `.specify/memory/constitution.md`
**Mode**: Read-only consistency, coverage & quality analysis
...
| L4 | Constraint Note | LOW | plan.md 複雜度追蹤 ↔ TC-005 | T061 統計 SSE 採「伺服器端 2 秒輪詢」推送，已於 plan.md 複雜度追蹤中以「server-side polling ≠ client polling」說明合規性；屬可接受設計取捨。 | 無需處理；建議於 T073c 量測時加註：若實測延遲 ≥ 4 s 觸發效能改善任務（升級為 in-process pub/sub 或縮短輪詢間隔）。 |
...
| FR-006a CSV 批次匯入 | ✅ | T029, T029a, T033 | CSV header 規範待補（U2） |
| FR-006b 手動逐筆新增 | ✅ | T028 | |
| FR-007 Session 四狀態 | ✅ | T042, T044, T045, T047 | |
| FR-007a 手動關閉 | ✅ | T044 | |
| FR-007b 自動逾時 | ✅ | T043, T047, T047a | 採懶惰檢查（T072 併入 T047） |
...
| FR-019 出席率（含跨日 UTC+8） | ✅ | T052, T053, T060 | ~~UTC 儲存衝突（I1）~~ 已修復 |
...
| FR-024 稽核日誌雙軌 | ✅ | T013, T016, T068, T068a, T069, T070 | 7 種事件（含 `session_opened`）皆覆蓋 |
| FR-025 兩學年保留 | ✅ | T004, T075 | 由 Volume + 備份策略支撐 |
| SC-001 課程 + CSV ≤ 120 s | ⚠️ | T073（手動） | 無自動量測（C1） |
| SC-002 點名 ≤ 30 s | ✅ | T073d | |
| SC-003 QR 更新 ≤ 1 s | ✅ | T073b | |
| SC-004 100 並發 | ✅ | T073a | |
| SC-005 統計 ≤ 5 s | ✅ | T073c | |
| SC-006 匯出 ≤ 30 s | ⚠️ | T073（手動） | 無自動量測（C1） |
| SC-007 過期 QR 100% 拒絕 | ✅ | T040 | |
| SC-008 統計頁 ≤ 60 s | ⚠️ | T073（手動） | 無自動量測（C1） |
| SC-009 99.5% 可用性 | ✅ | T074, T075 | 健康檢查 + 部署備份 |

> 圖例：✅ 充分覆蓋；⚠️ 有覆蓋但仍有 finding；❌ 完全缺失（本次無）。
...
```text
# 1. 補 C1 / U1 / A1 任務（建議由人工於 tasks.md 補列）
edit specs/001-course-attendance-rollcall/spec.md   # US5 / FR-009 措辭
edit specs/001-course-attendance-rollcall/tasks.md  # 新增 T073e / f / g

# 2. 補 U2 / L2（contracts/）：CSV header 規範、錯誤回應對映
edit specs/001-course-attendance-rollcall/contracts/api-routes.md

# 3. 重跑 analyze 驗證
/speckit-analyze
```

進入實作前**至少應確認 I1 修復未回退**；其餘 MEDIUM / LOW 可在實作 PR 中順帶處理而不至於延誤 MVP。

---

## Remediation Offer

需要我針對前述 finding（特別是 C1 / U1 / A1 / U2 / L2）提出**具體的編輯片段建議**（含 before / after 文字）嗎？回覆「是」我會列出 top-N 改寫文字，但**不會直接修改檔案**（仍由你最後確認後執行）。
